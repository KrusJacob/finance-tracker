...
import { formatMoney, startOfMonth, startOfWeek, startOfYear, totalsForRange } from "@/lib/finance";

function StatBlock({ title, transactions, from }: { title: string; transactions: Transaction[]; from?: Date }) {
      ...
      <CardContent className="flex flex-col gap-4 p-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="flex flex-col gap-1">
            ...
            <span className="text-lg font-semibold text-income tabular-nums">{formatMoney(income)}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="flex items-center gap-1 text-xs text-muted-foreground">
            ...
            <span className="text-lg font-semibold text-expense tabular-nums">{formatMoney(expense)}</span>
          </div>
        </div>
      </CardContent>
    </Card>
...
export function StatsSection({ transactions }: { transactions: Transaction[] }) {
  return (
    <section aria-label="Статистика" className="grid grid-cols-1 gap-4 md:grid-cols-3">
      ...
      <StatBlock title="За год" transactions={transactions} from={startOfYear()} />
    </section>
  );
}
